    ...
    FOR prod IN SELECT id FROM public.products LOOP
        ...
        FOR wh_rec IN SELECT id FROM public.warehouses LOOP
            ...
            BEGIN
                ...
                WHERE pii.product_id = prod.id AND pi.warehouse_id = wh_rec.id AND pi.status = 'posted';
                q_in := q_in + temp_val;
                ...
                WHERE sri.product_id = prod.id AND sr.warehouse_id = wh_rec.id AND sr.status = 'posted';
                q_in := q_in + temp_val; -- الجمع التراكمي الصحيح
                ...
                WHERE pri.product_id = prod.id AND pr.warehouse_id = wh_rec.id AND pr.status = 'posted';
                q_out := q_out + temp_val;
                ...
                WHERE sai.product_id = prod.id AND sa.warehouse_id = wh_rec.id AND sa.status = 'posted';

                -- ز. التحويلات (Transfers)
                SELECT COALESCE(SUM(sti.quantity), 0) INTO q_transfer_in FROM public.stock_transfer_items sti JOIN public.stock_transfers st ON sti.stock_transfer_id = st.id WHERE sti.product_id = prod.id AND st.to_warehouse_id = wh_rec.id AND st.status = 'posted';
                SELECT COALESCE(SUM(sti.quantity), 0) INTO q_transfer_out FROM public.stock_transfer_items sti JOIN public.stock_transfers st ON sti.stock_transfer_id = st.id WHERE sti.product_id = prod.id AND st.from_warehouse_id = wh_rec.id AND st.status = 'posted';
