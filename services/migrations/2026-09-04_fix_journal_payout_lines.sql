-- ==============================================================================
-- TriPro ERP — Patch: Fix generate_shift_closing_entry — Remove duplicate cash credit
-- Date: 2026-09-04
-- المشكلة: قيد الإغلاق يُدرج "دائن النقدية" لكل سحب، لكن v_actual_cash يعكس
--           الرصيد الفعلي بعد السحب بالفعل، فيحدث خلل في التوازن.
-- الحل: في حلقة المسحوبات نُدرج فقط مدين الوجهة (مصروف/عهدة/خزينة).
--       لا نُدرج دائن الدرج — لأن مدين النقدية الفعلية يغطي ذلك.
--
-- القيد الصحيح:
--   مدين: الوجهة (مصروف/عهدة/خزينة)  = قيمة السحب
--   [لا يوجد دائن نقدية — الدرج يُسوَّى بالرصيد الفعلي]
-- ==============================================================================
...
BEGIN
    ...
    -- الفارق الفعلي (actual - expected)
    -- expected = opening + cash_sales - petty_cash - cash_returns
    ...
    -- ── 5. السحوبات — مدين الوجهة فقط (بدون دائن الدرج) ─────
    --
    -- ملاحظة محاسبية مهمة:
    -- v_actual_cash = الرصيد الفعلي الذي بقي في الدرج بعد كل العمليات.
    -- وبما أننا سنُدرج مدين النقدية الفعلية في البند 6، فلا يوجد
    -- حاجة لإدراج "دائن الدرج" هنا — وإلا يختل التوازن.
    -- المعادلة: مبيعات = نقدية فعلية + مصروفات/سحوبات + مردودات
    --
    FOR v_payout_record IN (
        ...
        ELSE -- VAULT_TRANSFER / SAFE_DROP / غيره
        ...
        IF v_target_acc IS NOT NULL THEN
            -- مدين: الوجهة (مصروف / عهدة / خزينة أخرى)
            -- لا يُدرج "دائن الدرج" لأن v_actual_cash يعكس الصافي بعد السحوبات
            INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
    ...
    -- يُمثّل الرصيد الذي أحضره الكاشير فعلياً (بعد السحوبات والمرتجعات)
