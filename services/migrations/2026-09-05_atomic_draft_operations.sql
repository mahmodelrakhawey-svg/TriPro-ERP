-- =====================================================================
-- 🛡️ TriPro-ERP: Atomic Draft Invoice Operations
-- Date: 2026-09-05
-- Purpose:
--   1. Provide single-roundtrip, atomic database transactions for saving
--      and editing draft sales & purchase invoices with their items.
--   2. Prevent data loss, missing lines, and orphaned records when
--      network disconnects during multi-step REST saves.
-- =====================================================================

-- 1. دالة الحفظ والتعديل الذري لمسودات فواتير المبيعات مع بنودها
...
BEGIN
    -- استخراج أو توليد معرف الفاتورة
    ...
    -- إذا كانت الفاتورة موجودة مسبقاً، نقوم بتحديثها، وإلا نقوم بإنشائها
    ...
    -- مسح البنود القديمة بأمان وإعادة إدراج البنود الجديدة في نفس المعاملة الذرية
...
-- 2. دالة الحفظ والتعديل الذري لمسودات فواتير المشتريات مع بنودها
...
-- 3. منح الصلاحيات اللازمة للمستخدمين المصادق عليهم
...
GRANT EXECUTE ON FUNCTION public.save_purchase_invoice_draft(jsonb, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.save_purchase_invoice_draft(jsonb, jsonb) TO anon;
