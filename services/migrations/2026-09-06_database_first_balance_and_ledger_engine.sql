...
BEGIN
    ...
    manual_entries AS (
        ...
        JOIN public.journal_entries je 
          ON je.organization_id = p_org_id 
         AND je.status = 'posted'
         AND je.description ILIKE '%' || c.name || '%'
        WHERE c.organization_id = p_org_id
          AND c.deleted_at IS NULL
    ...
    opening_check AS (
        SELECT 
            ace.customer_id,
            COUNT(*) FILTER (WHERE je.description ILIKE '%رصيد افتتاحي%' OR je.reference ILIKE 'OP-%' OR je.reference ILIKE 'OB-%') AS op_count
        FROM all_customer_entries ace
...
BEGIN
    ...
    entry_to_customer AS (
        ...
        UNION ALL
        -- مستخلصات المقاولات (ربط آمن عبر جدول المشاريع projects)
        SELECT ppb.related_journal_entry_id AS je_id, p.customer_id
        FROM public.project_progress_billings ppb
        JOIN public.projects p ON p.id = ppb.project_id
    ...
    -- 3. القيود اليومية اليدوية والتسويات المذكور فيها اسم العميل (مطابق لكشف الحساب)
    manual_entries AS (
        SELECT je.id AS je_id, c.id AS customer_id
        FROM public.customers c
        JOIN public.journal_entries je 
    ...
    -- 4. صافي الرصيد الدفتري من قيود الأستاذ العام (مطابق 100% لكشف الحساب)
    ledger_balances AS (
        SELECT 
            ace.customer_id,
            COALESCE(SUM(jl.debit), 0) - COALESCE(SUM(jl.credit), 0) AS net_balance
    ...
    -- التحقق إذا كان الرصيد الافتتاحي له قيد يومية مسجل بالفعل
    opening_check AS (
        SELECT 
            ace.customer_id,
            COUNT(*) FILTER (WHERE je.description ILIKE '%رصيد افتتاحي%' OR je.reference ILIKE 'OP-%' OR je.reference ILIKE 'OB-%') AS op_count
    ...
    inv_items AS (
        SELECT 
            ...
            GREATEST(0::numeric, i.total_amount - COALESCE(i.paid_amount, 0)) AS amount
        FROM public.invoices i
        WHERE i.organization_id = p_org_id
          AND i.customer_id IS NOT NULL
    ...
    scaled AS (
        SELECT 
            ...
            ROUND(GREATEST(0::numeric, fm.raw_0_30 * 
                CASE WHEN fm.total_gross > 0 THEN LEAST(1::numeric, fm.real_balance / fm.total_gross) ELSE 0 END), 2) AS r_0_30,
            ROUND(GREATEST(0::numeric, fm.raw_31_60 * 
                CASE WHEN fm.total_gross > 0 THEN LEAST(1::numeric, fm.real_balance / fm.total_gross) ELSE 0 END), 2) AS r_31_60,
            ROUND(GREATEST(0::numeric, fm.raw_61_90 * 
                CASE WHEN fm.total_gross > 0 THEN LEAST(1::numeric, fm.real_balance / fm.total_gross) ELSE 0 END), 2) AS r_61_90
...
BEGIN
    ...
    -- 2. خريطة ربط القيود بالموردين من كافة المستندات
    entry_to_supplier AS (
        ...
        WHERE (ch.organization_id = p_org_id OR ch.organization_id IS NULL)
          ...
          AND ch.party_id IS NOT NULL
        UNION ALL
        -- هـ. الشيكات الصادرة (بمطابقة الاسم)
        ...
        UNION ALL
        -- و. مستخلصات مقاولي الباطن المرتبطة بالموردين
        SELECT sb.related_journal_entry_id AS je_id, s.id AS supplier_id
        FROM public.subcontractor_billings sb
        JOIN public.subcontractor_contracts sc ON sc.id = sb.contract_id
        JOIN public.subcontractors sub ON sub.id = sc.subcontractor_id
        JOIN public.suppliers s 
          ON s.organization_id = p_org_id 
         AND (s.id = sub.id OR TRIM(LOWER(s.name)) = TRIM(LOWER(sub.name)))
        WHERE sb.organization_id = p_org_id 
    ...
    -- 5. فترات الأعمار الزمنية من فواتير المشتريات ومستخلصات المقاولين
    inv_items AS (
        SELECT 
            pi.supplier_id,
        ...
        SELECT 
            ...
            GREATEST(0::numeric, sb.net_amount) AS amount
        FROM public.subcontractor_billings sb
        JOIN public.subcontractor_contracts sc ON sc.id = sb.contract_id
        JOIN public.subcontractors sub ON sub.id = sc.subcontractor_id
        JOIN public.suppliers s 
    ...
    SELECT 
        ...
        s.r_31_60 AS range_31_60,
        s.r_61_90 AS range_61_90,
        -- الفئة الأخيرة تضمن تطابق مجموع الفئات مع الرصيد الإجمالي 100% بدون أي هللة أو قرش فرق
        GREATEST(0::numeric, ROUND(s.total_balance - s.r_0_30 - s.r_31_60 - s.r_61_90, 2)) AS range_90_plus,
        s.total_balance
    FROM scaled s
    ORDER BY s.total_balance DESC;
