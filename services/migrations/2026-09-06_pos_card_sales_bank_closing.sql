-- ==============================================================================
-- Migration: 2026-09-06_pos_card_sales_bank_closing.sql
-- Description: تحديث دالة إغلاق الوردية لتشمل مبيعات الفيزا والشبكة والترحيل لحساب البنك
-- ==============================================================================
...
BEGIN
    ...
    -- 🛡️ منع تكرار القيد (Idempotency) وحذف أي قيد سابق لنفس الوردية لإعادة ترحيلها بدقة
    DELETE FROM public.journal_entries 
    WHERE related_document_id = p_shift_id 
      AND related_document_type = 'shift';

    -- 1. إجمالي المسحوبات والمصروفات النثرية من الدرج
    ...
    -- 2. مرتجعات المبيعات النقدية من الدرج
    ...
    -- 3. تجميع كافة طلبات الوردية المكتملة (تشمل الكاش والفيزا والشبكة)
    ...
    WHERE  (
             ...
             OR (
                 ...
                 AND (o.organization_id = v_org_id OR o.organization_id IS NULL)
             )
           )
    ...
    SELECT
        ...
        -- إجمالي المقبوض نقداً (كاش الدرج)
        COALESCE((
            SELECT SUM(p.amount) FROM public.payments p
            WHERE  p.order_id IN (SELECT id FROM temp_shift_orders)
              AND  (UPPER(p.payment_method) = 'CASH' OR p.payment_method LIKE '%نقد%' OR p.payment_method LIKE '%كاش%')
              AND  p.status = 'COMPLETED'
        ), 0) AS cash_total,
        -- إجمالي المقبوض فيزا وشبكة وبطاقات إلكترونية (البنك)
        COALESCE((
            SELECT SUM(p.amount) FROM public.payments p
            WHERE  p.order_id IN (SELECT id FROM temp_shift_orders)
              AND  UPPER(p.payment_method) NOT IN ('CASH')
              AND  p.payment_method NOT LIKE '%نقد%'
              AND  p.payment_method NOT LIKE '%كاش%'
              AND  p.status = 'COMPLETED'
        ), 0) AS card_total,
    ...
    -- 🛡️ [الضريبة ورسوم الخدمة]:
    v_service_total := COALESCE(v_summary.service_charge_sum, 0);

    IF v_summary.tax > 0 THEN
        v_vat_total := v_summary.tax;
    ELSIF ((v_summary.cash_total + v_summary.card_total) - (v_summary.subtotal + v_service_total)) > 0 THEN
        v_vat_total := ((v_summary.cash_total + v_summary.card_total) - (v_summary.subtotal + v_service_total));
    ELSE
        v_vat_total := 0;
    END IF;
    ...
    v_total_discount := GREATEST(
        ...
        GREATEST(0, (v_summary.subtotal + v_service_total + v_vat_total) - (v_summary.cash_total + v_summary.card_total))
    );

    IF (v_summary.subtotal + v_vat_total + v_service_total) <= ((v_summary.cash_total + v_summary.card_total) + 0.01) THEN
        v_gross_sales := v_food_sales_subtotal + v_total_discount;
    ELSE
        v_gross_sales := v_food_sales_subtotal;
    END IF;

    -- 🛡️ [النقدية الفعلية وفارق الدرج]:
    -- إذا لم يُدخل الكاشير النقدية الفعلية (NULL) نعتبر المتوقع هو الفعلي لمنع عجز وهمي
    IF v_shift.actual_cash IS NULL THEN
        v_actual_cash := GREATEST(0, COALESCE(v_shift.opening_balance, 0) + v_summary.cash_total - v_petty_cash - v_cash_returns);
    ELSE
        v_actual_cash := v_shift.actual_cash;
    END IF;

    -- الفارق الفعلي للدرج النقدي يعتمد على الكاش فقط دون أن يتأثر بمبيعات الفيزا
    v_diff := v_actual_cash
              - (COALESCE(v_shift.opening_balance, 0) + v_summary.cash_total - v_petty_cash - v_cash_returns);

    -- استرجاع خريطة الحسابات
    SELECT account_mappings INTO v_mappings
    FROM   public.company_settings WHERE organization_id = v_org_id;

    -- حساب الصندوق النقدي (الدرج)
    ...
    -- حساب البنك (لمبيعات الفيزا والشبكة)
    v_bank_acc_id := public.resolve_leaf_account(COALESCE(
        public.safe_cast_uuid(v_mappings->>'BANK'),
        public.safe_cast_uuid(v_mappings->>'BANK_ACCOUNTS'),
        (SELECT id FROM public.accounts WHERE organization_id = v_org_id AND code IN ('1232','123201','1102') LIMIT 1),
        (SELECT id FROM public.accounts WHERE organization_id = v_org_id AND (name LIKE '%بنك%' OR name LIKE '%فيزا%' OR name LIKE '%شبكة%') AND is_group = false LIMIT 1),
        v_cash_acc_id
    ));
    ...
    -- وصف القيد
    v_entry_desc := 'إغلاق وردية مبيعات التجزئة والماركت (مبيعات نقدية وفيزا)';
    ...
    IF v_vat_total > 0 THEN
        INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
        VALUES (v_je_id, v_vat_acc_id, 0, v_vat_total, 'ضريبة القيمة المضافة للوردية (VAT)', v_org_id);
    END IF;
    ...
    -- ── 6. المسحوبات والمصروفات النثرية (مدين) ───────────────
    ...
    -- ── 8. فروقات الدرج النقدي (عجز / زيادة حقيقية) ────────
    IF v_diff < 0 THEN
        INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
    ...
    -- ── 9. متحصلات مبيعات الفيزا والشبكة لحساب البنك (مدين) ──
    IF COALESCE(v_summary.card_total, 0) > 0 AND v_bank_acc_id IS NOT NULL THEN
        INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
        VALUES (v_je_id, v_bank_acc_id, v_summary.card_total, 0, 'متحصلات مبيعات فيزا وشبكة بنكية للوردية', v_org_id);
    END IF;

    -- ── 10. تكلفة البضاعة المباعة وصرف المخزون ───────────────
    IF COALESCE(v_summary.cost_total, 0) > 0 THEN
        FOR v_item_cost_record IN (
            ...
            IF v_item_cost_record.total_cost > 0 AND v_item_cost_record.inv_acc IS NOT NULL THEN
                INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
                VALUES (v_je_id, v_cogs_acc_id, v_item_cost_record.total_cost, 0, 'تكلفة مبيعات الوردية (COGS)', v_org_id);
                INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
