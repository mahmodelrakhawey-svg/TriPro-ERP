-- ==============================================================================
-- TriPro ERP — Migration: Cash Drop Types (EXPENSE / CUSTODIAN / VAULT_TRANSFER)
-- Date: 2026-09-04
-- ==============================================================================

-- 1. إضافة الأعمدة الجديدة في pos_petty_cash_payouts
ALTER TABLE public.pos_petty_cash_payouts
    ADD COLUMN IF NOT EXISTS public_shift_id UUID;
...
CREATE INDEX IF NOT EXISTS idx_petty_cash_public_shift_id
    ON public.pos_petty_cash_payouts(public_shift_id);

-- 2. تحديث generate_shift_closing_entry()
...
BEGIN
    ...
    -- 7. قيود المسحوبات — بند مستقل لكل سحب حسب نوعه
    FOR v_payout_record IN (
        ...
        IF v_target_acc IS NOT NULL THEN
            INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
            VALUES (v_je_id, public.resolve_leaf_account(v_target_acc), v_payout_record.amount, 0, v_line_desc, v_org_id);

            INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, organization_id)
            VALUES (v_je_id, v_cash_acc_id, 0, v_payout_record.amount, 'دفع من الدرج — ' || v_line_desc, v_org_id);
        END IF;
    END LOOP;
