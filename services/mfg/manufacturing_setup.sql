...
-- إضافة اسم بديل للتوافق (Compatibility Alias) لضمان عدم تعطل التقارير القديمة
...
-- يتم تعريف الجداول والأعمدة هنا "قبل" الرؤية لضمان صحة التبعيات
...
WITH labor_costs AS (
    SELECT production_order_id, SUM(COALESCE(labor_cost_actual, 0)) as total_labor
    FROM public.mfg_order_progress
    GROUP BY production_order_id
),
usage_costs AS (
    SELECT op.production_order_id, 
           SUM(amu.actual_quantity * COALESCE(p.weighted_average_cost, p.cost, p.purchase_price, 0)) as total_usage
    FROM public.mfg_actual_material_usage amu
    JOIN public.mfg_order_progress op ON amu.order_progress_id = op.id
...
SELECT 
    ...
    COALESCE(lc.total_labor, 0) as actual_labor,
    (COALESCE(uc.total_usage, 0) + COALESCE(rc.total_request, 0)) as actual_material,
    (COALESCE(lc.total_labor, 0) + COALESCE(uc.total_usage, 0) + COALESCE(rc.total_request, 0)) as total_actual_cost,
    ROUND((po.quantity_to_produce * COALESCE(NULLIF(p.sales_price, 0), NULLIF(p.price, 0), 0)) - 
          (COALESCE(lc.total_labor, 0) + COALESCE(uc.total_usage, 0) + COALESCE(rc.total_request, 0)), 2) as net_profit,
    CASE 
        WHEN (po.quantity_to_produce * COALESCE(NULLIF(p.sales_price, 0), NULLIF(p.price, 0), 0)) > 0
        THEN ROUND((((po.quantity_to_produce * COALESCE(NULLIF(p.sales_price, 0), NULLIF(p.price, 0), 0)) - 
                    (COALESCE(lc.total_labor, 0) + COALESCE(uc.total_usage, 0) + COALESCE(rc.total_request, 0))) / 
                    (po.quantity_to_produce * COALESCE(NULLIF(p.sales_price, 0), NULLIF(p.price, 0), 0)) * 100), 2)
        ELSE 0 
    END as margin_percentage,
    po.organization_id
FROM public.mfg_production_orders po
JOIN public.products p ON po.product_id = p.id
LEFT JOIN labor_costs lc ON po.id = lc.production_order_id
LEFT JOIN usage_costs uc ON po.id = uc.production_order_id
