...
├─ resolve_leaf_account (SQL Engine) (Architectural Fix)
                                    ─────────
...
## 🛡️ تحديث حاسم: محرك توجيه الحسابات الذكي (V51.0)
**المشكلة:** فشل القيود الآلية عند محاولة الترحيل إلى حسابات "مجموعات" (Group Accounts) بدلاً من الحسابات الفرعية.
**الحل المنفذ:** 
- بناء دالة `public.resolve_leaf_account(uuid)` في محرك النظام الموحد.
- الدالة تستخدم استعلاماً تكرارياً (Recursive CTE) للبحث في شجرة الحسابات والوصول لأول حساب "ورقة" (Leaf) متاح.
- تم تحصين مديولات (المطاعم، التصنيع، المقاولات، التكاليف المتقدمة) باستخدام هذه الدالة.
**النتيجة:** نجاح كافة الاختبارات الوحدوية (Unit Tests) بنسبة 100% ✅.
