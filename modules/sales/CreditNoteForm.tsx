      ...
      <form onSubmit={handleSave} className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 space-y-6 print:hidden">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div className="relative">
                    <input type="number" required min="0.01" step="any" value={formData.amount} onChange={e => setFormData({...formData, amount: parseFloat(e.target.value)})} className="w-full border rounded-lg px-4 py-2.5 font-bold" />
                ...
                <div className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-200">
                    <div className="flex items-center gap-3">
                        ...
                        <label htmlFor="isTaxable" className="cursor-pointer select-none">
                            ...
                            <span className="block text-xs text-slate-500">قم بإلغاء التحديد إذا كان الإشعار لخصم تجاري مسموح به أو تسوية بدون ضريبة</span>
                        </label>
                    </div>
                    <span className={`text-xs font-bold px-3 py-1 rounded-full ${formData.isTaxable ? 'bg-red-100 text-red-700' : 'bg-slate-200 text-slate-700'}`}>
        ...
        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-wrap gap-4 justify-between items-center text-sm font-bold">
            ...
            <div className="text-slate-600">
                الإجمالي النهائي: <span className="text-red-600 text-xl font-mono">{totalAmount.toLocaleString()}</span>
            </div>
        </div>
