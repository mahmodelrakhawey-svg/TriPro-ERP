...
export const QuotationPrint: React.FC<QuotationPrintProps> = ({ quoteData, companySettings }) => {
  if (!quoteData) return null;

  const items = quoteData.items || quoteData.quotation_items || [];
  const taxAmount = Number(quoteData.taxAmount ?? quoteData.tax_amount ?? 0);
  const totalAmount = Number(quoteData.totalAmount ?? quoteData.total_amount ?? 0);
  const subtotal = totalAmount - taxAmount;

  return (
    <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black font-sans" dir="rtl" id="printable-quotation">
      <style>{`
        @media print {
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-6">
        ...
        <div className="text-center">
            <h1 className="text-3xl font-black text-slate-900 mb-1">عرض أسعار</h1>
            <p className="text-base font-bold text-slate-500 uppercase tracking-widest">Price Quotation / Offer</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-200 mb-6">
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">رقم عرض السعر</span>
            <span className="text-lg font-black font-mono text-teal-700">{quoteData.quotationNumber || quoteData.quotation_number}</span>
        </div>
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">تاريخ العرض</span>
            <span className="text-base font-bold">{quoteData.date || quoteData.quotation_date}</span>
        </div>
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">العميل / الجهة</span>
            <span className="text-base font-bold">{quoteData.customerName || quoteData.customers?.name || '-'}</span>
        </div>
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">تاريخ سريان العرض</span>
            <span className="text-base font-bold text-red-600">{quoteData.expiryDate || quoteData.expiry_date || 'غير محدد'}</span>
        </div>
        {quoteData.notes && (
          <div className="col-span-4">
              <span className="block text-xs text-slate-500 font-bold mb-0.5">ملاحظات وشروط العرض</span>
              <span className="text-sm text-slate-700">{quoteData.notes}</span>
          </div>
        )}
      </div>
      ...
      <table className="w-full mb-6 text-right border border-slate-200">
        <thead className="bg-slate-100 border-b-2 border-slate-800 text-slate-800 font-bold text-sm">
            <tr>
                ...
                <th className="py-2.5 px-3">اسم الصنف / البيان</th>
        ...
        <tbody className="divide-y divide-slate-200 text-sm">
            {items.map((item: any, index: number) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                    ...
                    <td className="py-2 px-3 text-center font-bold text-teal-700 font-mono">{Number(item.quantity || 0).toLocaleString()}</td>
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="w-1/3 space-y-2 text-sm">
            ...
            <div className="flex justify-between text-base font-black text-teal-800 border-t-2 border-slate-800 pt-2">
      ...
      {/* Signatures */}
      <div className="grid grid-cols-2 gap-8 pt-10 border-t border-slate-200 text-center text-xs font-bold text-slate-600 mt-12">
          <div>
              <p className="mb-8">إدارة المبيعات والتسويق</p>
              <div className="border-b border-dashed border-slate-400 w-36 mx-auto"></div>
          </div>
          <div>
              <p className="mb-8">توقيع وختم العميل بالقبول والتعميد</p>
              <div className="border-b border-dashed border-slate-400 w-36 mx-auto"></div>
          </div>
      </div>
    </div>
