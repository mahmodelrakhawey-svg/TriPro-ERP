...
export const SalesReturnPrint: React.FC<SalesReturnPrintProps> = ({ returnData, companySettings }) => {
  if (!returnData) return null;

  const items = returnData.items || returnData.sales_return_items || [];
  ...
  return (
    <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black font-sans" dir="rtl" id="printable-sales-return">
      <style>{`
        @media print {
          body * { visibility: hidden; }
          #printable-sales-return, #printable-sales-return * { visibility: visible; }
          #printable-sales-return { position: absolute; left: 0; top: 0; width: 100%; }
        }
      `}</style>
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-6">
        ...
        <div className="text-center">
            <h1 className="text-3xl font-black text-slate-900 mb-1">مرتجع مبيعات</h1>
            <p className="text-base font-bold text-slate-500 uppercase tracking-widest">Sales Return / Credit Receipt</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-200 mb-6">
        <div>
            ...
            <span className="text-lg font-black font-mono text-red-600">{returnData.returnNumber || returnData.return_number}</span>
        </div>
        <div>
        ...
        <div>
            ...
            <span className="text-base font-bold">{returnData.customerName || returnData.customers?.name || returnData.customer?.name || 'عميل نقدي'}</span>
        </div>
        <div>
        ...
        {returnData.originalInvoiceNumber || returnData.invoices?.invoice_number ? (
          <div className="col-span-2">
              <span className="block text-xs text-slate-500 font-bold mb-0.5">مرتجع من فاتورة رقم</span>
              <span className="text-base font-mono font-bold text-blue-600">{returnData.originalInvoiceNumber || returnData.invoices?.invoice_number}</span>
          </div>
        ) : (
          <div className="col-span-2">
        ...
        <tbody className="divide-y divide-slate-200 text-sm">
            {items.map((item: any, index: number) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                    ...
                    <td className="py-2 px-3 text-center font-bold text-red-600 font-mono">{Number(item.quantity || 0).toLocaleString()}</td>
                    <td className="py-2 px-3 text-center font-mono">{Number(item.unitPrice || item.unit_price || item.price || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="w-1/3 space-y-2 text-sm">
            ...
            <div className="flex justify-between text-base font-black text-red-700 border-t-2 border-slate-800 pt-2">
                <span>إجمالي المرتجع المستحق:</span>
      ...
      <div className="grid grid-cols-3 gap-8 pt-10 border-t border-slate-200 text-center text-xs font-bold text-slate-600 mt-12">
          <div>
              <p className="mb-8">أمين المستودع (المستلم)</p>
          ...
          <div>
              <p className="mb-8">إدارة المبيعات / الاعتماد</p>
              <div className="border-b border-dashed border-slate-400 w-32 mx-auto"></div>
          </div>
      </div>
    </div>
