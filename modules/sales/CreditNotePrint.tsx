...
export const CreditNotePrint: React.FC<CreditNotePrintProps> = ({ noteData, companySettings }) => {
  ...
  return (
    <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black font-sans" dir="rtl" id="printable-credit-note">
      <style>{`
        @media print {
          body * { visibility: hidden; }
          #printable-credit-note, #printable-credit-note * { visibility: visible; }
          #printable-credit-note { position: absolute; left: 0; top: 0; width: 100%; }
        }
      `}</style>
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-6">
        ...
        <div className="text-center">
            <h1 className="text-3xl font-black text-slate-900 mb-1">إشعار دائن (تسوية عميل)</h1>
            <p className="text-base font-bold text-slate-500 uppercase tracking-widest">Credit Note / Adjustment Voucher</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-200 mb-6">
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">رقم الإشعار الدائن</span>
            <span className="text-lg font-black font-mono text-purple-700">{noteData.noteNumber || noteData.credit_note_number}</span>
        </div>
        <div>
        ...
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">اسم العميل</span>
            <span className="text-base font-bold">{noteData.customerName || noteData.customers?.name || '-'}</span>
        </div>
        <div>
      ...
      <table className="w-full mb-6 text-right border border-slate-200">
        <thead className="bg-slate-100 border-b-2 border-slate-800 text-slate-800 font-bold text-sm">
            <tr>
                ...
                <th className="py-2.5 px-3 text-center">إجمالي الإشعار الدائن</th>
            </tr>
        </thead>
        <tbody className="divide-y divide-slate-200 text-sm">
            <tr className="bg-white">
                <td className="py-4 px-3 font-bold text-slate-900">
                  {noteData.notes || 'تسوية رصيد وتخفيض مديونية العميل'}
                </td>
                ...
                <td className="py-4 px-3 text-center font-mono font-black text-purple-700 text-base">
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="w-1/3 space-y-2 text-sm">
            ...
            <div className="flex justify-between text-base font-black text-purple-900 border-t-2 border-slate-800 pt-2">
                <span>إجمالي التخفيض المعتمد:</span>
      ...
      <div className="grid grid-cols-3 gap-8 pt-10 border-t border-slate-200 text-center text-xs font-bold text-slate-600 mt-12">
          ...
          <div>
              <p className="mb-8">توقيع وختم العميل بالعلم</p>
              <div className="border-b border-dashed border-slate-400 w-32 mx-auto"></div>
          </div>
      </div>
    </div>
