...
import { 
  RotateCcw, Search, Plus, Trash2, Edit, Printer, Download, 
  MessageCircle, Loader2, Filter, AlertCircle, ChevronLeft, 
  ChevronRight, ArrowRight, Eye, RefreshCw, FileText
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import * as XLSX from 'xlsx';
import { SalesReturnPrint } from './SalesReturnPrint';

export const SalesReturnsList: React.FC = () => {
  const { warehouses, customers, currentUser, settings, selectedFiscalYear, fiscalYearRange } = useAccounting();
      ...
      let query = supabase
        ...
        .select(`
          *,
          ...
          sales_return_items(id, product_id, quantity, unit_price, total, uom_id, products(name, sku))
        `)
  ...
  useEffect(() => {
    fetchReturns();
  }, [startDate, endDate, selectedWarehouseId, selectedCustomerId, statusFilter]);
  ...
  const filteredReturns = useMemo(() => {
    ...
    return returns.filter(r => 
      ...
      (r.invoices?.invoice_number && r.invoices.invoice_number.toLowerCase().includes(term)) ||
  ...
  const handleDelete = async (ret: any) => {
    if (!window.confirm(`هل أنت متأكد من حذف مرتجع المبيعات رقم ${ret.return_number}؟\nسيتم إلغاء حركة المخزون والقيد المحاسبي بالكامل.`)) {
      return;
    }
    ...
    try {
      ...
      // 1. إذا كان المرتجع مرحلاً، نعكس المخزون
      if (ret.status === 'posted' && ret.sales_return_items && ret.sales_return_items.length > 0) {
        for (const item of ret.sales_return_items) {
          if (item.product_id && item.quantity) {
            // خصم الكمية التي كانت قد أضيفت عند الإرجاع
      ...
      if (ret.related_journal_entry_id) {
        ...
        // البحث بالمرجع إن لم يكن مربوطاً بالحقل المباشر
      ...
      // 3. حذف بنود المرتجع
      await supabase.from('sales_return_items').delete().eq('sales_return_id', ret.id);

      // 4. حذف سجل المرتجع
      const { error: delError } = await supabase.from('sales_returns').delete().eq('id', ret.id);
  ...
  const handleEdit = (ret: any) => {
    navigate('/sales-return', { state: { returnToEdit: ret } });
  };
  ...
  const handleWhatsApp = (ret: any) => {
    ...
    const message = `مرحباً ${ret.customers?.name || 'عميلنا العزيز'}،
تم تسجيل مرتجع مبيعات رقم: ${ret.return_number}
...
شكراً لتعاملكم معنا.`;
  ...
  const handleExportExcel = () => {
    const data = [
      ...
      [],
      ['رقم المرتجع', 'التاريخ', 'العميل', 'المستودع', 'الفاتورة الأصلية', 'المبلغ قبل الضريبة', 'الضريبة', 'الإجمالي النهائي', 'الحالة', 'ملاحظات'],
      ...filteredReturns.map(r => [
        r.return_number,
        r.return_date,
        r.customers?.name || 'عميل نقدي',
        r.warehouses?.name || '-',
        r.invoices?.invoice_number || 'مرتجع حر',
    ...
    XLSX.writeFile(wb, `Sales_Returns_${startDate}_${endDate}.xlsx`);
  };
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print:hidden">
        <div>
          <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2">
            <RotateCcw className="text-red-600" /> سجل مرتجعات المبيعات
          </h2>
          <p className="text-slate-500 text-sm">عرض، إدارة، تعديل وطباعة مرتجعات المبيعات وحركاتها المخزنية والمحاسبية</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button 
            onClick={() => navigate('/sales-return')} 
            className="bg-red-600 text-white px-4 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-sm hover:bg-red-700 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:hidden">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-black text-red-600 mt-1" dir="ltr">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-wrap items-end gap-3 print:hidden">
        <div className="flex-1 min-w-[220px]">
          <label className="block text-xs font-bold text-slate-600 mb-1">بحث سريع (رقم المرتجع / العميل / الفاتورة)</label>
          <div className="relative">
            <input 
              ...
              className="w-full border rounded-xl p-2.5 pl-10 bg-slate-50 focus:bg-white text-sm outline-none focus:border-red-500 transition-colors"
            />
        ...
        <div className="w-full sm:w-auto min-w-[160px]">
          <label className="block text-xs font-bold text-slate-600 mb-1">العميل</label>
          <select 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          <div className="p-16 text-center">
            <Loader2 className="animate-spin mx-auto text-red-600 mb-2" size={36} />
            <p className="text-slate-500 font-bold">جاري تحميل سجل المرتجعات...</p>
          </div>
          ...
          <div className="p-16 text-center text-slate-500 flex flex-col items-center gap-3">
            ...
            <h4 className="text-lg font-bold text-slate-700">لا توجد مرتجعات مسجلة</h4>
            <p className="text-sm text-slate-400 max-w-sm">لم يتم العثور على أي مرتجعات مبيعات مطابقة لمعايير البحث الحالية.</p>
            <button 
              onClick={() => navigate('/sales-return')} 
              className="mt-2 bg-red-600 text-white px-5 py-2 rounded-xl font-bold text-sm hover:bg-red-700 transition-colors flex items-center gap-1.5"
            >
                    ...
                    <tr key={ret.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="p-4">
                        <span 
                          ...
                          className="font-mono font-black text-red-600 hover:underline cursor-pointer flex items-center gap-1"
                        >
                          {ret.return_number}
                        </span>
                      </td>
                      ...
                      <td className="p-4 text-center font-bold text-slate-700">
                        {ret.sales_return_items?.length || 0}
                      </td>
                      <td className="p-4 text-center font-mono font-black text-red-600" dir="ltr">
