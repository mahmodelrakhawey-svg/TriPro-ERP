/**
 * ملف جديد: سجل سندات الصرف مع نظام الصفحات
 * المسار: modules/finance/PaymentVoucherList.tsx
 */
...
// Define the interface for Payment Voucher
...
const PaymentVoucherList = () => {
  ...
  const handleExportExcel = () => {
      const exportData = vouchers.map(v => ({
          ...
          'المورد / المستفيد': v.suppliers?.name || 'غير محدد',
          'المبلغ': v.amount,
          'البيان': v.notes || '-'
      }));
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2">
            <ArrowUpRight className="text-red-600" /> سجل سندات الصرف
          </h2>
          <p className="text-slate-500 font-medium">أرشيف كامل لجميع عمليات الصرف النقدي والبنكي</p>
        </div>
        <div className="flex gap-2">
             <button onClick={() => navigate('/payment-voucher')} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-md">
                <Plus size={18} />
                <span>سند جديد</span>
            </button>
        ...
        <table className="w-full text-right">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs font-black uppercase tracking-widest">
                <tr>
                    ...
                    <th className="py-5 px-6">المورد / المستفيد</th>
            ...
            <tbody className="divide-y divide-slate-100">
                {vouchers.map(voucher => (
                    <tr key={voucher.id} className="hover:bg-slate-50/50 transition-colors group">
                        <td className="py-4 px-6 font-black text-red-600 font-mono">{voucher.voucher_number || '-'}</td>
                        <td className="py-4 px-6 text-slate-500 font-medium">{voucher.payment_date}</td>
                        <td className="py-4 px-6 font-bold text-slate-800">{voucher.suppliers?.name || 'غير محدد'}</td>
                        ...
                        <td className="py-4 px-6 text-center">
                            {voucher.payment_voucher_attachments && voucher.payment_voucher_attachments.length > 0 && (
                                <button onClick={() => handleViewAttachments(voucher.payment_voucher_attachments)} className="text-blue-600 hover:underline text-xs flex items-center justify-center gap-1">
                                    <Eye size={14} />
                ...
                {vouchers.length === 0 && !loading && (
                    <tr><td colSpan={8} className="p-12 text-center text-slate-400 font-medium">لا توجد سندات صرف مطابقة</td></tr>
                )}
            </tbody>
        </table>
