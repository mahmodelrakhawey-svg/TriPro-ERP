...
export const ExpenseVoucherPrint: React.FC<ExpenseVoucherPrintProps> = ({ voucher, companySettings }) => {
    ...
    <div className="hidden print:block p-8 bg-white text-black rtl w-full" dir="rtl">
      ...
      <div className="flex justify-between items-center border-b-2 border-slate-800 pb-4 mb-6">
        <div className="text-right">
          <h1 className="text-2xl font-black">{companySettings?.company_name || companySettings?.companyName || 'سند صرف مصروفات'}</h1>
      ...
      <div className="border-2 border-slate-100 rounded-2xl p-6 mb-8 bg-slate-50/30 text-right">
        <div className="space-y-6">
          <div className="flex items-center gap-2">
            <span className="font-bold whitespace-nowrap">يصرف للسيد/ة:</span>
            <span className="border-b border-dotted border-slate-400 flex-1 px-2">{voucher.recipientName || '---'}</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="border-b border-dotted border-slate-400 flex-1 px-2">{voucher.description || voucher.expenseAccountName || 'مصروفات متنوعة'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-16 grid grid-cols-2 gap-20 text-center">
        ...
        <div>
          <p className="font-bold border-t border-slate-800 pt-2">توقيع المحاسب / الختم</p>
        </div>
      </div>
