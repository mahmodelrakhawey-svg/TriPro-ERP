...
export const PaymentVoucherPrint: React.FC<PaymentVoucherPrintProps> = ({ voucher, companySettings }) => {
  ...
  const isSupplier = Boolean(voucher.supplier_id || voucher.party_id || voucher.subType === 'supplier');
  const partyName = voucher.suppliers?.name || voucher.recipient_name || voucher.partyName || (isSupplier ? '---' : 'مصروفات عامة');
  const notes = voucher.notes || voucher.description || (isSupplier ? 'سداد مستحقات مورد' : 'صرف مصروفات');
  const currency = voucher.currency || companySettings?.currency || 'EGP';
  const voucherTitle = isSupplier ? 'سند صرف للمورد' : 'سند صرف مصروفات';
