...
export const PaymentVoucherPrint = ({ voucher, companySettings }: PaymentVoucherPrintProps) => {
    ...
    <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black font-sans" dir="rtl" id="printable-voucher">
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-8">
        ...
        <div className="text-center">
            <h1 className="text-3xl font-black text-slate-900 mb-2">سند صرف</h1>
            <p className="text-lg font-bold text-slate-500 uppercase tracking-widest">Payment Voucher</p>
        </div>
        <div className="text-left">
             {/* Logo Placeholder if URL exists */}
      ...
      <div className="flex justify-between mb-8">
        <div className="flex gap-4">
            <div className="bg-slate-100 px-4 py-2 rounded border border-slate-200">
                ...
                <span className="text-xl font-black font-mono">{voucher.voucher_number}</span>
            </div>
            <div className="bg-slate-100 px-4 py-2 rounded border border-slate-200">
                <span className="block text-xs text-slate-500 font-bold">التاريخ</span>
                <span className="text-xl font-bold">{voucher.payment_date}</span>
            </div>
        </div>
        <div className="bg-slate-100 px-6 py-2 rounded border border-slate-200 text-center min-w-[200px]">
             ...
             <span className="text-2xl font-black text-slate-900">{Number(voucher.amount).toLocaleString()} <span className="text-sm text-slate-500">{voucher.currency || 'EGP'}</span></span>
             <p className="text-xs text-slate-600 mt-1 font-bold border-t border-slate-300 pt-1">{tafqeet(Number(voucher.amount), voucher.currency || 'EGP')}</p>
        </div>
      </div>
      ...
      <div className="border border-slate-300 rounded-lg p-6 space-y-6 mb-12">
        <div className="flex items-baseline gap-4">
            <span className="font-bold text-slate-700 w-32 shrink-0">يصرف للسيد/ة:</span>
            <div className="flex-1 border-b border-dotted border-slate-400 pb-1 font-bold text-lg">{voucher.suppliers?.name || '---'}</div>
        </div>
        
        <div className="flex items-baseline gap-4">
            <span className="font-bold text-slate-700 w-32 shrink-0">وذلك عن:</span>
            <div className="flex-1 border-b border-dotted border-slate-400 pb-1">{voucher.notes || '---'}</div>
        </div>
        ...
        <div className="flex items-baseline gap-4">
            ...
            <div className="flex-1 border-b border-dotted border-slate-400 pb-1">
                {voucher.payment_method === 'cash' ? 'نقدي' : 
                 ...
                 voucher.payment_method === 'transfer' ? 'تحويل بنكي' : voucher.payment_method}
            </div>
        </div>
      </div>
