...
export const PurchaseOrderPrint: React.FC<PurchaseOrderPrintProps> = ({ orderData, companySettings }) => {
  if (!orderData) return null;

  const items = orderData.items || orderData.purchase_order_items || [];
  ...
  return (
    <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black font-sans" dir="rtl" id="printable-purchase-order">
      <style>{`
        @media print {
          body * { visibility: hidden; }
          #printable-purchase-order, #printable-purchase-order * { visibility: visible; }
          #printable-purchase-order { position: absolute; left: 0; top: 0; width: 100%; }
        }
      `}</style>
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-6">
        ...
        <div className="text-center">
            <h1 className="text-3xl font-black text-slate-900 mb-1">أمر شراء (توريد)</h1>
            <p className="text-base font-bold text-slate-500 uppercase tracking-widest">Purchase Order</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-200 mb-6">
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">رقم أمر الشراء</span>
            <span className="text-lg font-black font-mono text-blue-600">{orderData.orderNumber || orderData.po_number}</span>
        </div>
        <div>
        ...
        <div>
            ...
            <span className="text-base font-bold">{orderData.supplierName || orderData.suppliers?.name || '-'}</span>
        </div>
        <div>
            <span className="block text-xs text-slate-500 font-bold mb-0.5">تاريخ التسليم المتوقع</span>
            <span className="text-base font-bold">{orderData.deliveryDate || orderData.expected_delivery_date || 'غير محدد'}</span>
        </div>
        {orderData.notes && (
          <div className="col-span-4">
              <span className="block text-xs text-slate-500 font-bold mb-0.5">ملاحظات / شروط التوريد</span>
      ...
      <table className="w-full mb-6 text-right border border-slate-200">
        <thead className="bg-slate-100 border-b-2 border-slate-800 text-slate-800 font-bold text-sm">
            <tr>
                ...
                <th className="py-2.5 px-3 text-center">السعر التقديري</th>
        ...
        <tbody className="divide-y divide-slate-200 text-sm">
            {items.map((item: any, index: number) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                    ...
                    <td className="py-2 px-3 font-bold text-slate-900">{item.name || item.products?.name || 'N/A'}</td>
                    <td className="py-2 px-3 text-center text-slate-600">{item.uomName || item.uoms?.name || '-'}</td>
                    <td className="py-2 px-3 text-center font-bold text-blue-600 font-mono">{Number(item.quantity || 0).toLocaleString()}</td>
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="w-1/3 space-y-2 text-sm">
            ...
            <div className="flex justify-between text-base font-black text-blue-700 border-t-2 border-slate-800 pt-2">
      ...
      <div className="grid grid-cols-3 gap-8 pt-10 border-t border-slate-200 text-center text-xs font-bold text-slate-600 mt-12">
          <div>
              <p className="mb-8">مسؤول المشتريات</p>
          ...
          <div>
              <p className="mb-8">استلام المورد وتأكيد التوريد</p>
              <div className="border-b border-dashed border-slate-400 w-32 mx-auto"></div>
          </div>
      </div>
    </div>
