    ...
    if (this.state.hasError) {
      return (
        <div style={{ padding: '40px', fontFamily: 'sans-serif', direction: 'rtl', textAlign: 'center', backgroundColor: '#f8fafc', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <p style={{ color: '#475569', marginBottom: '24px' }}>يرجى محاولة تحديث الصفحة. إذا استمرت المشكلة، قد تكون البيانات المخزنة مؤقتاً في المتصفح تالفة.</p>
          ...
          <button 
            onClick={() => {
                if(window.confirm('هل ترغب في مسح الذاكرة المؤقتة للمتصفح وإعادة تشغيل النظام؟')) {
                    try {
                        localStorage.clear();
